...
// Stdio MCP server that forwards tool calls to the Debug app of one worktree.
// The app publishes bin/debug-mcp.json while it runs; this bridge validates
// that file so a stale entry or another worktree's instance is never used.
...
async Task<JsonNode?> HandleAsync(JsonObject request)
{
    ...
    {
        case "initialize":
            var requested = (request["params"] as JsonObject)
                ?["protocolVersion"]
                ?.GetValue<string>();
            return RpcResult(
                id,
                DebugMcpContract.InitializeResult(
                    "software-crawler-debug-bridge",
                    "SoftwareCrawler Worktree Debug Bridge",
                    "1",
                    requested
                )
            );
        ...
        return RpcResult(
            ...
            ToolError(
                $"No usable Debug instance for workspace '{workspace}'. "
                    + $"Build and launch this worktree (Run.cmd), then retry. {ex.Message}"
            )
        );
    }
}
...
DebugMcpDiscovery ReadAndValidateDiscovery()
{
    ...
    var discovery =
        JsonSerializer.Deserialize<DebugMcpDiscovery>(File.ReadAllText(discoveryPath))
...
static string ResolveWorkspace(string[] commandLine)
{
    ...
    while (
        ...
        && !File.Exists(Path.Combine(directory.FullName, "SoftwareCrawler.slnx"))
    )
