...
- `bin/Config/` — user data only, git-ignored apart from the `.gitkeep` that keeps the folder present (its existence is what selects portable mode). `LocalSettings.tab` holds this machine's enabled flags, download directories and frequent-check flags and has no version control; a Debug build reads and writes `Templates/Software.tab` directly, a released build works on a copy seeded into `Config/`. `ScheduleState.json` records when the last full run finished — regenerable, losing it just costs one extra crawl.
