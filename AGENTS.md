...
- `bin/Config/` — the shared user data, git-ignored apart from the `.gitkeep` that keeps the folder present (its existence is what selects portable mode). It holds the roaming `settings.json` and, for a released build, the `Software.tab` copy seeded from the template; a Debug build reads and writes `Templates/Software.tab` directly.
- `%LOCALAPPDATA%\SoftwareCrawler\Config\` — everything that is true of this computer alone, and of no other: the machine `settings.json`, `LocalSettings.tab` (enabled flags, download directories, frequent-check flags — no version control and no second copy), `DownloadHistory.tab` (per item, when it was last checked and how old the file now in its download directory is) and `ScheduleState.json` (when the last full run finished). The last two are regenerable. Older builds kept the three under `bin/Config/`; `MachineDataMigration` moves them on startup, and a new machine-local file has to be registered there too.
...
- App tools: `control_tree`, `screenshot`, `software_list`, `download_probe`, `download_batch`, `script_edit`, `page_state`, `storage_info`, `config_monitor`, `schedule`.
