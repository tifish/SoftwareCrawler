...
**浏览器窗口的显示归主界面那个按钮管**（`MainForm.Resident.cs`）：它是个开关，按当前状态在 "Show browser" 与 "Hide browser" 之间换标题，点一下把浏览器摆到屏幕上，再点一下挪回屏幕外。位置由两个条件相与决定：用户要不要看（`_browserWanted`）**且**主窗口在用。所以后台轮次绝不会在屏幕上支起一个浏览器，从托盘打开主窗口也不会顺带把它拽出来；收回托盘时它自己跟着退场，再打开时回到用户上次的选择。`_browserWanted` 的初值取决于启动方式：普通启动为真（保持配方维护者一直以来看得见浏览器的习惯），`--tray` 启动为假。
...
**两个下载目录，一个够用就干活**（`DownloadPipeline.ResolveDownloadDirectories`）：开跑前两个目录各探一次（`DownloadDirectoryAccess`，探不通的共享会被记为不可达、后续项直接快速失败）。两个都不通才判该项失败；只有一个不通就带着另一个继续，最终只落到那个可用的目录，并在错误信息列里留一句 `Skipped: ...`（该项仍算成功）——一台机器的共享掉线不该让另一份也拿不到。主目录不通时，第二目录**整份接管**：文件、元数据、下载历史、钩子脚本都以它为准（`_primaryDirectory` 就是这一轮的"目标目录"）。
...
扩展点：目标目录下若存在 `AfterDownload.cmd`/`.ps1` 或 `AfterExtract.cmd`/`.ps1`，会以文件路径为参数同步调用（`.cmd` 优先），并在子进程环境里放 `SOFTWARECRAWLER_EVENT`（`AfterDownload`/`AfterExtract`）、`SOFTWARECRAWLER_ITEM`（软件名）、`SOFTWARECRAWLER_FILE`（同第一个参数）、`SOFTWARECRAWLER_DIRECTORY`（下载目录）。用环境变量而不是再加一个参数位，是为了让按老约定写的脚本原样继续工作；脚本据此能只处理刚到的这一个文件，而不是把整个目录当成都该处理——比如上传脚本，清单里的另一个文件可能还没下载过，全量处理会把"文件不存在"算成失败。脚本限时 10 分钟、7-Zip 限时 30 分钟，超时或取消本项时连同子进程整棵杀掉并按失败处理——一个等不到结果的脚本（比如托盘程序不理会不带 `/F` 的 `taskkill`）否则会让整批永远停在这一项。解压用随程序附带的 `bin/7-Zip/7z.exe`，默认用 `x -r` 保留压缩包中的目录结构；只有配方显式设置 `ExtractToRoot` 时才用 `e -r` 展平到根目录，并清理空子目录。`.tar.gz`/`.tgz` 里套着一层 tar，7-Zip 一次只剥一层，所以要跑第二遍再删掉中间的 `.tar`——否则下载目录里会多出一个配方没要求保留的归档。若配置了 `FilePatternToDeleteBeforeExtraction`，解压前先按该模式删除目标目录顶层的旧文件——抽出的安装包文件名常带版本号，否则新旧会并存。每个下载完成的归档都会写入 `.softwarecrawler-download-metadata.json`（按软件名保存源 URL、文件名、大小和 `Last-Modified`）；一旦该项元数据存在，后续更新判断只比较元数据，不再比较保留在目录中的归档。归档在开启了 `ExtractAfterDownload` 并成功解压后删除（等 `AfterExtract` 跑完再删），与是否调用了脚本无关——要解压的配方要的是内容而不是归档。没开解压时，即使脚本成功也保留归档：脚本（上传、离线安装）往往还要用到归档本身。成功执行过解压或任一脚本后，元数据里记 `Processed`；同一版本再次遇到仍保留着的归档时，只有未标记 `Processed` 才重跑解压和脚本（之前某一步失败了），否则不再动它——不然保留归档会让脚本每轮都跑一遍。新版本下载时该标记重置。

这两类外部进程都**不显示控制台窗口、检查退出码、失败时把输出记进日志**（`RunProcessAsync`）。运行期间状态显示为 `Extracting` 或 `RunningEventScript`，成功后回到 `Downloaded`/`SameFileAlreadyDownloaded` 这个结束状态。失败即视为该项失败且不重试——文件已经在盘上，重下没有意义；错误信息以 `When Extracting:`/`When RunningEventScript:` 开头，能看出是哪一步。7-Zip 的退出码 1 是非致命警告，按成功处理，2 及以上才算失败。
...
**门禁**（`CanRunNow`）只有两条，两种轮次一视同仁：已有批次在跑（下载严格串行、共用一个浏览器），以及 `UserPresence.IsBusy()`（`SHQueryUserNotificationState` 报告全屏 / 投影 / 勿扰）。注意锁屏（`QUNS_NOT_PRESENT`）**不算忙**——那恰恰是最好的时机。

**主窗口开着不是条件**。曾经是（先是两种轮次都拦，后来只拦全量），结果是高频只在没人看的时候跑——你去看它的那一刻正是它不工作的那一刻。功能观察不到就没法被信任，所以这条整个去掉了。代价是：你开着窗口干活时，到点的全量会照跑并占住浏览器十几分钟；真要拦住它，用取消，或者把那个时刻从时间表里去掉。

**两套时间表对坏时机的反应相同：丢掉，不补跑。** 到期就推进各自的时间戳，不管这一趟跑没跑成，下一个时刻才是下一次机会。全量也一样——曾经是"延后"（`_lastFullRun` 不动、每个 tick 重问直到能跑），撤回了：那意味着一趟被挡下的全量会在几分钟后毫无预兆地复活，抢走浏览器十几分钟，而那时用户早已在干别的事了。

> **被跳过的全量不能把高频一起拖下水。** `TickAsync` 判完全量后**继续往下走**，不 `return`。曾经是 return 的，于是 18:30 的全量等用户关窗口一直等到 22:27，这四个小时里高频一次都没跑过——每个 tick 都在全量那个分支返回了。改成"跳过"以后这个窗口小多了，但只要两件事还在同一个 tick 里判，坑就还在，所以到期与否由 `GetDueRuns` 独立判定，并有测试盯着这条不变式。

`NoteSkip` 只在原因**变化**时记日志：一天 100 多轮，同一条原因重复几十遍会把日志淹掉。
