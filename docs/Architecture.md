...
> **延后的全量不能把高频一起拖下水。** `TickAsync` 判完全量后**继续往下走**，不 `return`。曾经是 return 的，于是 18:30 的全量等用户关窗口一直等到 22:27，这四个小时里高频一次都没跑过——每个 tick 都在全量那个分支返回了。窗口这条门禁虽然已经去掉，但只要还存在任何会让全量延后的原因（批次在跑、全屏），这个坑就还在，所以两者到期与否由 `GetDueRuns` 独立判定，并有测试盯着这条不变式。
