...
**门禁**（`CanRunNow`）是"不打扰"的落点：已有批次在跑、`UserPresence.IsBusy()`（`SHQueryUserNotificationState` 报告全屏 / 投影 / 勿扰）——这两条两种轮次都拦。注意锁屏（`QUNS_NOT_PRESENT`）**不算忙**——那恰恰是最好的时机。

**主窗口开着只拦全量**。全量要占住浏览器十几分钟，用户没法同时干活；而高频只跑用户点名的几项、几秒钟，它的结果本来就是要刷进用户正看着的表格里。这一条最初两种轮次都拦，结果是**只要有人看着，高频就永远不跑**——这个功能没法被观察到，也就没法被信任。
...
> **延后的全量不能把高频一起拖下水。** `TickAsync` 判完全量后**继续往下走**，不 `return`。曾经是 return 的，于是 18:30 的全量等用户关窗口一直等到 22:27，这四个小时里高频一次都没跑过——每个 tick 都在全量那个分支返回了。两者到期与否由 `GetDueRuns` 独立判定，有测试盯着这条不变式。
